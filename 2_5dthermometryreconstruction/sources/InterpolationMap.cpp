...
InterpolationMap::InterpolationMap(CoordinatesConverter _world2VolumeConverter)
{
	...
	//TODO check what is right vtk unsigend char int etc
	vesselMap->AllocateScalars(VTK_UNSIGNED_CHAR, 1);
	...
	for (int z = 0; z < 100; z++)
	{
		...
		for (int x = 0; x < 256; x++)
		{
			for (int y = 0; y < 256; y++)
			{
				...
				//uchar* vslPxl = static_cast<uchar*>(vesselMap_origin->GetScalarPointer(x, y, z));
				//vslPxl[0] = tmpPxl[0];
...
void InterpolationMap::createPolarCoordinateMap()
{
	...
	//qDebug() << "createPolarCoordinateMap - Done";
}

void InterpolationMap::populateInterpolationMap(QString _fileName)
{
	qDebug() << "populateInterpolationMap";
	...
	for (int angleNr = 0; angleNr < sizeof(slices_angle); angleNr++)
	{
		...
		//QString fileDataName = _fileName + "/" + slices_angle.at(angleNr) +".dcm";
		qDebug() << fileDataName;
	...
	//qDebug() << "populateInterpolationMap - Done";
