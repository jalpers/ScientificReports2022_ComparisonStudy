...
void DataVolume::addSlice(QString _angle, QString _timeStep)
{
	...
	{
		// TODO
		// Consideration of heat distribution during the process. Comparing actual slice with last loaded slice at the same angle. Transfer this information to other slices
	...
	volume->Modified();
	qDebug() << "DataVolume::addSlice - Done";
}
...
void DataVolume::interpolate(std::vector<std::vector<InterpolationMap::PixelProperties>>interpolationMap2D, vtkSmartPointer<vtkImageData> _vesselMap)//
{
	...
	//qDebug() << "DataVolume::Interpolate - Done";
...
void DataVolume::preprocessed(vtkSmartPointer<vtkImageData> imageDicom)
{
	//Median Filter
	vtkSmartPointer<vtkImageHybridMedian2D> hybridMedian =
		vtkSmartPointer<vtkImageHybridMedian2D>::New();
	hybridMedian->SetInputData(imageDicom);
	hybridMedian->Update();
	imageDicom = hybridMedian->GetOutput();
	qDebug() << "preproceesd0";
}
