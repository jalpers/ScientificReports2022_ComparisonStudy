...
public:
	...
	*/
	void interpolate(std::vector<std::vector<InterpolationMap::PixelProperties>> _interpolationMap2D, vtkSmartPointer<vtkImageData> _vesselMap);//TODO Interpolation map in klasse auslagern und bei Interpolate als Parameter übergeben
	//InterpolationMap _interpolationMap
	// vtkSmartPointer<vtkImageData> _vesselMap = nullptr
	
	
	vtkSmartPointer<vtkImageData> getImageData();
	
	void preprocessed(vtkSmartPointer<vtkImageData> imageDicom);
