...
class EllipsoidTool:
    ...
    def plotEllipsoid(self, center, radii, rotation, ax=None, plotAxes=False, cageColor='b', cageAlpha=0.2):
        ...
        for i in range(len(x)):
            for j in range(len(x)):
                #l = np.dot([x[i,j],y[i,j],z[i,j]], rotation)
    ...
    def plotEllipsoid2D(self, center, radii, rotation, ax=None, plotAxes=False, cageColor='b', cageAlpha=0.2):
        """Plot an ellipsoid"""
        make_ax = ax == None
        if make_ax:
            fig = plt.figure()
    ...
    def isInsideEllipsoid(self, coord, c, r,rot):
        cr = coord - c
        rr =(-1)* r ** cr
        s = 0
        s = pow(rot[0,0] * cr[0] + rot[1,0] * cr[1],2)/pow(r[0],2) + pow(rot[1,0] * cr[0] + rot[1,1] * cr[1],2)/pow(r[1],2)
        #for i in range(2):
            #s = s + pow(rr[i],2)/pow(r[i],2)
            #s = s + pow((coord[i]- c[i]),2)/pow(r[i],2)
        return s <= 1
